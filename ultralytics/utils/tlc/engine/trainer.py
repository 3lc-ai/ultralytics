...
class TLCTrainerMixin(BaseTrainer):
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        self._settings = (
            Settings() if "settings" not in overrides else overrides.pop("settings")
        )
        ...
        # Column names
        self._image_column_name = overrides.pop(
        ...
        if RANK in {-1, 0}:
            self._metrics_collection_epochs = set(
                self._settings.get_metrics_collection_epochs(self.epochs)
            )

            # Create a 3LC run
            description = (
                self._settings.run_description
                if self._settings.run_description
                else DEFAULT_TRAIN_RUN_DESCRIPTION
            )
            project_name = (
                self._settings.project_name
                if self._settings.project_name
                else self.data["train"].project_name
            )
            self._run = tlc.init(
    ...
    def _log_3lc_parameters(self):
        ...
        parameters = {
            **vars(self.args),  # YOLO arguments
            "3LC/train_url": str(
                self.data.get("train").url
            ),  # 3LC table used for training
    ...
    def _print_metrics_collection_epochs(self):
        ...
        # Print collection epochs
            ...
            if len(self._metrics_collection_epochs) == 1:
                ...
                message = (
                    f"Metrics will be collected after training and after epoch {epoch}."
                )
            else:
                epochs = ", ".join(
                    str(epoch) for epoch in sorted(self._metrics_collection_epochs)
                )
    ...
    def final_eval(self):
        ...
        if (
            not self._settings.collection_val_only
