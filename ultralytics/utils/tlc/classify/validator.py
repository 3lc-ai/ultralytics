...
class TLCClassificationValidator(
    TLCValidatorMixin, yolo.classify.ClassificationValidator
):
    ...
    def _get_metrics_schemas(self):
        ...
        predicted_value = tlc.Int32Value(
            value_map=predicted_value_map, number_role=tlc.NUMBER_ROLE_LABEL
        )
        ...
        column_schemas = {
            "loss": tlc.Schema(
                "Loss", "Cross Entropy Loss", writable=False, value=tlc.Float32Value()
            ),
    ...
    def _compute_3lc_metrics(self, preds, batch):
        ...
        batch_metrics = {
            ...
            "predicted": [
                self.data["range_to_3lc_class"][int(p)] for p in predicted.tolist()
            ],
            "confidence": confidence,
            "top1_accuracy": (torch.argmax(preds, dim=1) == batch["cls"]).to(
                torch.float32
            ),
        }

        if (
            len(self.dataloader.dataset.table.get_value_map(self._label_column_name))
            > 5
        ):
    ...
    def _add_embeddings_hook(self, model):
        ...
        if not has_linear_layer:
            raise ValueError(
                "No linear layer found in model, cannot collect embeddings."
            )
