...
class TLCDetectionValidator(TLCValidatorMixin, DetectionValidator):
    ...
    def _get_metrics_schemas(self):
        loss_schemas = (
            yolo_loss_schemas(training=self._training)
    ...
    def _compute_3lc_metrics(self, preds, batch):
        losses = (
            self.loss_fn(self._curr_raw_preds, batch)
            if self._settings.collect_loss
            else {}
        )
    ...
    def _process_detection_predictions(self, preds, batch):
        ...
        for i, predictions in enumerate(preds):
            ...
            if pbatch["bbox"].shape[0]:
                ious = metrics.box_iou(
                    pbatch["bbox"], pred_scaled
                )  # IoU evaluated in xyxy format
            ...
            assert len(annotations) <= self._settings.max_det, (
                "Should have at most MAX_DET predictions per image."
            )
    ...
    def _add_embeddings_hook(self, model) -> int:
        ...
        sppf_index = next(
            (i for i, m in enumerate(model.model) if "SPPF" in m.type), -1
        )
        ...
        def hook_fn(module, input, output):
            ...
            self_ref = weak_self()
            flattened_output = (
                torch.nn.functional.adaptive_avg_pool2d(output, (1, 1))
                .squeeze(-1)
                .squeeze(-1)
            )
        ...
        activation_size = (
            model.model[sppf_index]._modules["cv2"]._modules["conv"].out_channels
        )
