...
class TLCYOLODetectionDataset(BaseTLCYOLODataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        :param label_column_name: Name of the label column in the table
        """
        self._detection_factory: Callable[[list[float]], tlc.BoundingBox] = (
            self._get_detection_factory(table, label_column_name)
        )
    ...
    def _get_detection_factory(
        ...
        try:
            factory = tlc.BoundingBox.from_schema(
                table.rows_schema.values[column_name].values[instances_name]
            )
        except Exception as e:
            raise ValueError(f"Table {table.url} is not a detection table: {e}")
...
class TLCYOLOSegmentationDataset(BaseTLCYOLODataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        :param label_column_name: Name of the label column in the table
        """
        self._segment_type: SegmentType = self._get_segment_type(
    ...
    def _get_segment_type(
        ...
        try:
            ...
            raise ValueError(f"Table {table.url} is not a segmentation table: {e}")

        return segment_type

    def _normalize_segments(
        self, segments: list[np.ndarray], width: int, height: int
    ) -> list[np.ndarray]:
    ...
    def _get_label_from_row(
        ...
        for i, (category, polygon) in enumerate(
            ...
            if len(polygon) < 6:
                LOGGER.warning(
                    f"Polygon {i} in row {example_id} has fewer than 6 points and will be ignored."
                )
                continue
