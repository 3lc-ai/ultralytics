...
class TLCYOLODataset(TLCDatasetMixin, YOLODataset):
    ...
    def __init__(self, table, data=None, task="detect", exclude_zero=False, class_map=None, **kwargs):
        ...
        self.class_mapping = class_map if class_map is not None else IdentityDict()
    ...
    def get_labels(self):
        ...
        for example_id, row in enumerate(self.table.table_rows):
            ...
            self.labels.append(tlc_table_row_to_yolo_label(row, self._table_format, self.class_mapping))
...
def unpack_boxes(bboxes: list[dict[str, int | float]], class_mapping: dict[int, int]) -> tuple[np.ndarray, np.ndarray]:
    classes_list, boxes_list = [], []
    for bbox in bboxes:
        _class, box = unpack_box(bbox)
        classes_list.append(class_mapping[_class])
...
def tlc_table_row_to_yolo_label(row, table_format: str, class_mapping: dict[int, int]) -> dict[str, Any]:
    classes, bboxes = unpack_boxes(row[tlc.BOUNDING_BOXES][tlc.BOUNDING_BOX_LIST], class_mapping)
